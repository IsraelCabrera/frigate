...
const SecurityDateStamp = /\[[\d\s-:]*]/;
const SecuritySection = /[\w.]*/;
...
export function parseLogLines(logService: LogType, logs: string[]) {
  if (logService == "security") {
    return logs
      .map((line) => {
        const match = SecurityDateStamp.exec(line);
        ...
        const sectionMatch = SecuritySection.exec(
