...
from security.const import REGEX_HTTP_CAMERA_USER_PASS, REGEX_RTSP_CAMERA_USER_PASS
...
def update_yaml_file_bulk(file_path: str, updates: Dict[str, Any]):
    ...
    try:
        ...
        logger.error(
            f"Unable to read from Security config file {file_path}. Make sure it exists and is readable."
        )
        return
    ...
    try:
        ...
        logger.error(f"Unable to write to Security config file {file_path}: {e}")
