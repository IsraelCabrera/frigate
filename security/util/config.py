...
from security.const import CONFIG_DIR, EXPORT_DIR
from security.util.services import get_video_properties
...
def migrate_security_config(config_file: str):
    """handle migrating the security config."""
    logger.info("Checking if security config needs migration...")
    ...
    if previous_version == CURRENT_CONFIG_VERSION:
        logger.info("security config does not need migration...")
        return
    ...
    if previous_version < "0.14":
        logger.info(f"Migrating security config from {previous_version} to 0.14...")
    ...
    if previous_version < "0.15-0":
        logger.info(f"Migrating security config from {previous_version} to 0.15-0...")
    ...
    if previous_version < "0.15-1":
        logger.info(f"Migrating security config from {previous_version} to 0.15-1...")
    ...
    if previous_version < "0.16-0":
        logger.info(f"Migrating security config from {previous_version} to 0.16-0...")
    ...
    if previous_version < "0.17-0":
        logger.info(f"Migrating security config from {previous_version} to 0.17-0...")
        new_config = migrate_017_0(config)
        with open(config_file, "w") as f:
            yaml.dump(new_config, f)
        previous_version = "0.17-0"

    logger.info("Finished security config migration...")


def migrate_014(config: dict[str, dict[str, Any]]) -> dict[str, dict[str, Any]]:
    """Handle migrating security config to 0.14"""
...
def migrate_015_0(config: dict[str, dict[str, Any]]) -> dict[str, dict[str, Any]]:
    """Handle migrating security config to 0.15-0"""
    new_config = config.copy()
...
def migrate_015_1(config: dict[str, dict[str, Any]]) -> dict[str, dict[str, Any]]:
    """Handle migrating security config to 0.15-1"""
    new_config = config.copy()
...
def migrate_016_0(config: dict[str, dict[str, Any]]) -> dict[str, dict[str, Any]]:
    """Handle migrating security config to 0.16-0"""
    new_config = config.copy()
