...
class AudioTranscriptionConfig(SecurityBaseModel):
...
class BirdClassificationConfig(SecurityBaseModel):
...
class CustomClassificationStateCameraConfig(SecurityBaseModel):
    crop: list[float, float, float, float] = Field(
        title="Crop of image frame on this camera to run classification on."
    )


class CustomClassificationStateConfig(SecurityBaseModel):
...
class CustomClassificationObjectConfig(SecurityBaseModel):
...
class CustomClassificationConfig(SecurityBaseModel):
...
class ClassificationConfig(SecurityBaseModel):
...
class SemanticSearchConfig(SecurityBaseModel):
...
class TriggerConfig(SecurityBaseModel):
    friendly_name: Optional[str] = Field(
        None, title="Trigger friendly name used in the Security UI."
    )
...
class CameraSemanticSearchConfig(SecurityBaseModel):
...
class FaceRecognitionConfig(SecurityBaseModel):
...
class CameraFaceRecognitionConfig(SecurityBaseModel):
    enabled: bool = Field(default=False, title="Enable face recognition.")
    min_area: int = Field(
        default=750, title="Min area of face box to consider running face recognition."
    )
...
class ReplaceRule(SecurityBaseModel):
...
class LicensePlateRecognitionConfig(SecurityBaseModel):
...
class CameraLicensePlateRecognitionConfig(SecurityBaseModel):
    enabled: bool = Field(default=False, title="Enable license plate recognition.")
...
class CameraAudioTranscriptionConfig(SecurityBaseModel):
    enabled: bool = Field(default=False, title="Enable audio transcription.")
