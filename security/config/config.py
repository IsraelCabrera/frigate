...
from security.const import REGEX_JSON
from security.detectors import DetectorConfig, ModelConfig
from security.detectors.detector_config import BaseDetectorConfig
from security.plus import PlusApi
from security.util.builtin import (
    deep_merge,
    get_ffmpeg_arg_list,
)
from security.util.config import (
    StreamInfoRetriever,
...
from security.util.image import create_mask
from security.util.services import auto_detect_hwaccel
...
def verify_valid_live_stream_names(
    security_config: SecurityConfig, camera_config: CameraConfig
) -> ValueError | None:
    """Verify that a restream exists to use for live view."""
    for _, stream_name in camera_config.live.streams.items():
        if (
            stream_name
            not in security_config.go2rtc.model_dump().get("streams", {}).keys()
        ):
            return ValueError(
...
def verify_lpr_and_face(
    ...
    if camera_config.lpr.enabled and not security_config.lpr.enabled:
        raise ValueError(
    ...
    if (
        ...
        and not security_config.face_recognition.enabled
    ):
        raise ValueError(
...
class SecurityConfig(SecurityBaseModel):
    version: Optional[str] = Field(default=None, title="Current config version.")
    safe_mode: bool = Field(
        default=False, title="If Security should be started in safe mode."
    )
    ...
    environment_vars: EnvVars = Field(
        default_factory=dict, title="Security environment variables."
    )
    ...
    def post_validation(self, info: ValidationInfo) -> Self:
        ...
        if self.plus_api and not self.snapshots.clean_copy:
            logger.warning(
                "Security+ is configured but clean snapshots are not enabled, submissions to Security+ will not be possible./"
            )
    ...
    @classmethod
    def load(cls, **kwargs):
        """Loads the Security config file, runs migrations, and creates the config object."""
        ...
        if not os.path.isfile(config_path):
            ...
            migrate_security_config(config_path)
        ...
        with open(config_path, "a+" if new_config else "r") as f:
            ...
            f.seek(0)
            return SecurityConfig.parse(f, **kwargs)
    ...
    def parse(cls, config, *, is_json=None, safe_load=False, **context):
        ...
        # load minimal Security config after the full config did not validate
        if safe_load:
