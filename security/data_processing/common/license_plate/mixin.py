...
from security.const import CLIPS_DIR, MODEL_CACHE_DIR
from security.embeddings.onnx.lpr_embedding import LPR_EMBEDDING_SIZE
from security.types import TrackedObjectUpdateTypesEnum
from security.util.builtin import EventsPerSecond, InferenceSpeed
from security.util.image import area
...
class LicensePlateProcessingMixin:
    ...
    def _detect_license_plate(
        ...
        """
        Use a lightweight YOLOv9 model to detect license plates for users without Security+
    ...
    def lpr_process(
        ...
        # dedicated LPR cam without security+
        if dedicated_lpr:
            id = "dedicated-lpr"
            ...
            # don't run for non car/motorcycle or non license plate (dedicated lpr with security+) objects
            if (
            ...
            if "license_plate" not in self.config.cameras[camera].objects.track:
                ...
                # don't run for object without attributes if this isn't dedicated lpr with security+
                if (
                ...
                # we are using dedicated lpr with security+
        ...
        # save the best snapshot for dedicated lpr cams not using security+
        if (
            dedicated_lpr
            and "license_plate" not in self.config.cameras[camera].objects.track
        ):
