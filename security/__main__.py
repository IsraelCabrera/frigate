...
from security.log import setup_logging
...
def main() -> None:
    ...
    threading.current_thread().name = "security"
    stop_event = mp.Event()
    ...
    try:
        config = SecurityConfig.load(install=True)
        ...
        # attempt to start Security in recovery mode
        try:
            config = SecurityConfig.load(install=True, safe_load=True)
            print("Starting Security in safe mode.")
        except ValidationError:
            print("Unable to start Security in safe mode.")
            sys.exit(1)
    ...
    SecurityApp(config, manager, stop_event).start()
...
if __name__ == "__main__":
    mp.set_forkserver_preload(
        [
            ...
            "sqlite3",
            # Third-party libraries commonly used in Security
            "numpy",
            "cv2",
            "peewee",
            "zmq",
