...
from security.api.defs.query.events_query_parameters import (
    DEFAULT_TIME_RANGE,
    EventsQueryParams,
    EventsSearchQueryParams,
    EventsSummaryQueryParams,
)
from security.api.defs.query.regenerate_query_parameters import (
    RegenerateQueryParameters,
)
from security.api.defs.request.events_body import (
    EventsCreateBody,
    EventsDeleteBody,
...
from security.api.defs.response.event_response import (
    EventCreateResponse,
    EventMultiDeleteResponse,
    EventResponse,
    EventUploadPlusResponse,
)
from security.api.defs.response.generic_response import GenericResponse
from security.api.defs.tags import Tags
from security.comms.event_metadata_updater import EventMetadataTypeEnum
from security.const import CLIPS_DIR, TRIGGER_DIR
from security.embeddings import EmbeddingsContext
from security.models import Event, ReviewSegment, Timeline, Trigger
from security.track.object_processing import TrackedObject
from security.util.file import get_event_thumbnail_bytes
from security.util.time import get_dst_transitions, get_tz_modifiers
...
def events_search(
    ...
    if not request.app.security_config.semantic_search.enabled:
        return JSONResponse(
            content=(
                {
                    "success": False,
...
@router.post(
    ...
    summary="Send event to Security+.",
    description="""Sends an event to Security+.
    Returns a success message or an error if the event is not found.
    """,
)
async def send_to_plus(request: Request, event_id: str, body: SubmitPlusBody = None):
    ...
    try:
        plus_id = request.app.security_config.plus_api.upload_image(image, event.camera)
    ...
    if include_annotation is not None:
        ...
        try:
            request.app.security_config.plus_api.add_annotation(
                event.plus_id,
                box,
                event.label,
            )
...
@router.put(
    ...
    response_model=EventUploadPlusResponse,
    summary="Submit false positive to Security+",
    description="""Submit an event as a false positive to Security+.
    This endpoint is the same as the standard Security+ submission endpoint,
...
async def false_positive(request: Request, event_id: str):
    ...
    if event.false_positive:
        message = "False positive already submitted to Security+"
        logger.error(message)
        return JSONResponse(
            content=({"success": False, "message": message}), status_code=400
        )
    ...
    try:
        request.app.security_config.plus_api.add_false_positive(
            event.plus_id,
            region,
            box,
            score,
...
async def set_description(
    ...
    if request.app.security_config.semantic_search.enabled:
        context: EmbeddingsContext = request.app.embeddings
...
async def regenerate_description(
    ...
    camera_config = request.app.security_config.cameras[event.camera]
...
def create_event(
    ...
    if not camera_name or not request.app.security_config.cameras.get(camera_name):
        return JSONResponse(
            content=(
...
def create_trigger_embedding(
    ...
    try:
        ...
        Trigger.create(
            ...
            model=request.app.security_config.semantic_search.model,
