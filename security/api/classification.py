...
from security.api.defs.request.classification_body import (
...
from security.api.defs.response.classification_response import (
...
from security.config.camera import DetectConfig
from security.const import CLIPS_DIR, FACE_DIR, MODEL_CACHE_DIR
from security.embeddings import EmbeddingsContext
from security.models import Event
from security.util.classification import (
    collect_object_classification_examples,
    collect_state_classification_examples,
    get_dataset_image_count,
    read_training_metadata,
)
from security.util.file import get_event_snapshot
...
def reclassify_face(request: Request, body: dict = None):
    if not request.app.security_config.face_recognition.enabled:
        return JSONResponse(
            status_code=400,
...
def train_face(request: Request, name: str, body: dict = None):
    ...
    if training_file_name:
        ...
        detect_config: DetectConfig = request.app.security_config.cameras[
            event.camera
        ].detect
...
async def register_face(request: Request, name: str, file: UploadFile):
    ...
    if not isinstance(result, dict):
        return JSONResponse(
            ...
            content={
                "success": False,
                "message": "Could not process request. Try restarting Security.",
            },
        )
...
def rename_face(request: Request, old_name: str, body: RenameFaceBody):
    ...
    try:
        ...
        return JSONResponse(
            status_code=400,
            content={
                "message": "Error renaming face. Check Security logs.",
                "success": False,
            },
        )
...
def reprocess_license_plate(request: Request, event_id: str):
    if not request.app.security_config.lpr.enabled:
...
def transcribe_audio(request: Request, body: AudioTranscriptionBody):
    ...
    if not request.app.security_config.cameras[event.camera].audio_transcription.enabled:
