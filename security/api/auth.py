...
from security.api.defs.request.app_body import (
    AppPostLoginBody,
    AppPostUsersBody,
    AppPutPasswordBody,
    AppPutRoleBody,
...
from security.config import AuthConfig, ProxyConfig
from security.const import CONFIG_DIR, JWT_SECRET_ENV_VAR, PASSWORD_HASH_ALGORITHM
...
def first_time_login(request: Request):
    ...
    """
    auth_config = request.app.security_config.auth
...
def get_remote_addr(request: Request):
    ...
    trusted_proxies = []
    for proxy in request.app.security_config.auth.trusted_proxies:
        try:
...
def require_role(required_roles: List[str]):
    async def role_checker(request: Request):
        proxy_config: ProxyConfig = request.app.security_config.proxy
        config_roles = list(request.app.security_config.auth.roles.keys())
...
def auth(request: Request):
    auth_config: AuthConfig = request.app.security_config.auth
    proxy_config: ProxyConfig = request.app.security_config.proxy

    success_response = Response("", status_code=202)

    # dont require auth if the request is on the internal port
    # this header is set by Security's nginx proxy, so it cant be spoofed
    ...
    JWT_COOKIE_NAME = request.app.security_config.auth.cookie_name
    JWT_COOKIE_SECURE = request.app.security_config.auth.cookie_secure
    JWT_REFRESH = request.app.security_config.auth.refresh_time
    JWT_SESSION_LENGTH = request.app.security_config.auth.session_length
...
def profile(request: Request):
    ...
    all_camera_names = set(request.app.security_config.cameras.keys())
    roles_dict = request.app.security_config.auth.roles
...
def login(request: Request, body: AppPostLoginBody):
    ...
    if verify_password(password, password_hash):
        ...
        config_roles_set = set(request.app.security_config.auth.roles.keys())
        ...
        if role == "admin":
            request.app.security_config.auth.admin_first_time_login = False
...
def create_user(
    ...
    HASH_ITERATIONS = request.app.security_config.auth.hash_iterations
    config_roles = list(request.app.security_config.auth.roles.keys())
