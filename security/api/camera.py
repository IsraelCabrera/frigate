...
from security.util.builtin import clean_camera_user_pass
from security.util.image import run_ffmpeg_snapshot
from security.util.services import ffprobe_stream
...
def go2rtc_camera_stream(request: Request, camera_name: str):
    ...
    if not r.ok:
        camera_config = request.app.security_config.cameras.get(camera_name)
...
def ffprobe(request: Request, paths: str = "", detailed: bool = False):
    ...
    if path_param.startswith("camera"):
        ...
        if camera not in request.app.security_config.cameras.keys():
            return JSONResponse(
                content=(
        ...
        if not request.app.security_config.cameras[camera].enabled:
            return JSONResponse(
        ...
        paths = map(
            ...
            request.app.security_config.cameras[camera].ffmpeg.inputs,
        )
    ...
    for path in paths:
        ffprobe = ffprobe_stream(
            request.app.security_config.ffmpeg, path.strip(), detailed=detailed
        )
