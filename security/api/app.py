...
from security.api.auth import require_role
from security.api.defs.query.app_query_parameters import AppTimelineHourlyQueryParameters
from security.api.defs.request.app_body import AppConfigSetBody
from security.api.defs.tags import Tags
from security.config import SecurityConfig
from security.config.camera.updater import (
    CameraConfigUpdateEnum,
    CameraConfigUpdateTopic,
)
from security.models import Event, Timeline
from security.stats.prometheus import get_metrics, update_metrics
from security.util.builtin import (
...
from security.util.services import (
    get_nvidia_driver_info,
    process_logs,
    restart_security,
    vainfo_hwaccel,
)
from security.util.time import get_tz_modifiers
...
def is_healthy():
    return "Security is running. Alive and healthy!"


@router.get("/config/schema.json")
def config_schema(request: Request):
    return Response(
        content=request.app.security_config.schema_json(), media_type="application/json"
    )
...
def config(request: Request):
    config_obj: SecurityConfig = request.app.security_config
    ...
    for camera_name, camera in request.app.security_config.cameras.items():
    ...
    config["plus"] = {"enabled": request.app.security_config.plus_api.is_active()}
    ...
    for detector_config in config["detectors"].values():
        detector_config["model"]["labelmap"] = (
            request.app.security_config.model.merged_labelmap
        )
...
def config_save(save_option: str, body: Any = Body(media_type="text/plain")):
    ...
    try:
        ...
        SecurityConfig.parse_yaml(new_config)
        ...
        return JSONResponse(
            content=(
                {
                    "success": False,
                    "message": "Your configuration is invalid.\nSee the official documentation at docs.security.video.\n\n"
                    + "\n".join(error_message),
                }
            ),
            status_code=400,
    ...
    try:
        ...
        return JSONResponse(
            content=(
                {
                    "success": False,
                    "message": "Could not write config file, be sure that Security has write permission on the config file.",
                }
            ),
            status_code=400,
        )
    ...
    if save_option == "restart":
        try:
            ...
            logging.error(f"Error restarting Security: {e}")
            return JSONResponse(
                content=(
                    {
                        "success": True,
                        "message": "Config successfully saved, unable to restart Security",
                    }
                ),
                status_code=200,
            )
...
def config_set(request: Request, body: AppConfigSetBody):
    ...
    try:
        ...
        try:
            config = SecurityConfig.parse(new_raw_config)
        except Exception:
            with open(config_file, "w") as f:
    ...
    if body.requires_restart == 0 or body.update_topic:
        old_config: SecurityConfig = request.app.security_config
        request.app.security_config = config
...
async def logs(
    service: str = Path(enum=["security", "nginx", "go2rtc"]),
    download: Optional[str] = None,
    stream: Optional[bool] = False,
    start: Optional[int] = 0,
    end: Optional[int] = None,
):
    """Get logs for the requested service (security/nginx/go2rtc)"""
    ...
    log_locations = {
        "security": "/dev/shm/logs/security/current",
    ...
    try:
        ...
        return JSONResponse(
            content=(
                {
                    "success": False,
                    "message": "Unable to restart Security.",
                }
            ),
            status_code=500,
        )
...
def plusModels(request: Request, filterByCurrentModelDetector: bool = False):
    if not request.app.security_config.plus_api.is_active():
        return JSONResponse(
            content=({"success": False, "message": "Security+ is not enabled"}),
            status_code=400,
        )

    models: dict[Any, Any] = request.app.security_config.plus_api.get_models()
    ...
    # current model type
    modelType = request.app.security_config.model.model_type

    # current detectorType for comparing to supportedDetectors
    detectorType = list(request.app.security_config.detectors.values())[0].type
