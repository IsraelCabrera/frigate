...
from security.api.auth import get_allowed_cameras_for_filter, require_camera_access
from security.api.defs.query.media_query_parameters import (
    Extension,
...
from security.camera.state import CameraState
...
from security.models import Event, Previews, Recordings, Regions, ReviewSegment
...
from security.util.image import get_image_from_recording
from security.util.time import get_dst_transitions
...
async def mjpeg_feed(
    ...
    if camera_name in request.app.security_config.cameras:
...
async def latest_frame(
    ...
    if camera_name in request.app.security_config.cameras:
        ...
        retry_interval = float(
            request.app.security_config.cameras.get(camera_name).ffmpeg.retry_interval
            or 10
        )
        ...
        if frame is None or datetime.now().timestamp() > (
            ...
            if request.app.camera_error_image is None:
                error_image = glob.glob(
                    os.path.join(INSTALL_DIR, "security/images/camera-error.jpg")
                )
        ...
        and request.app.security_config.birdseye.enabled
        and request.app.security_config.birdseye.restream
    ):
        frame = cv2.cvtColor(
...
async def get_snapshot_from_recording(
    ...
    if camera_name not in request.app.security_config.cameras:
        return JSONResponse(
            content={"success": False, "message": "Camera not found"},
            status_code=404,
        )
    ...
    if recording is not None:
        ...
        config: SecurityConfig = request.app.security_config
...
async def submit_recording_snapshot_to_plus(
    ...
    try:
        ...
        request.app.security_config.plus_api.upload_image(nd, camera_name)
...
def grid_snapshot(
    ...
    if camera_name in request.app.security_config.cameras:
        detect = request.app.security_config.cameras[camera_name].detect
        frame_processor: TrackedObjectProcessor = request.app.detected_frames_processor
...
def event_snapshot_clean(request: Request, event_id: str, download: bool = False):
    ...
    try:
        ...
        snapshot_config = request.app.security_config.cameras[event.camera].snapshots
