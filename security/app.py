...
from security.api.auth import hash_password
from security.api.fastapi_app import create_fastapi_app
from security.camera import CameraMetrics, PTZMetrics
from security.camera.maintainer import CameraMaintainer
from security.comms.base_communicator import Communicator
from security.comms.dispatcher import Dispatcher
from security.comms.event_metadata_updater import EventMetadataPublisher
from security.comms.inter_process import InterProcessCommunicator
from security.comms.mqtt import MqttClient
from security.comms.object_detector_signaler import DetectorProxy
from security.comms.webpush import WebPushClient
from security.comms.ws import WebSocketClient
from security.comms.zmq_proxy import ZmqProxy
from security.config.camera.updater import CameraConfigUpdatePublisher
...
from security.db.sqlitevecq import SqliteVecQueueDatabase
from security.embeddings import EmbeddingProcess, EmbeddingsContext
from security.events.audio import AudioProcessor
from security.events.cleanup import EventCleanup
from security.events.maintainer import EventProcessor
from security.log import _stop_logging
...
from security.output.output import OutputProcess
from security.ptz.autotrack import PtzAutoTrackerThread
from security.ptz.onvif import OnvifController
from security.record.cleanup import RecordingCleanup
from security.record.export import migrate_exports
from security.record.record import RecordProcess
from security.review.review import ReviewProcess
from security.stats.emitter import StatsEmitter
from security.stats.util import stats_init
from security.storage import StorageMaintainer
from security.timeline import TimelineProcessor
from security.track.object_processing import TrackedObjectProcessor
from security.util.builtin import empty_and_close_queue
from security.util.image import UntrackedSharedMemory
from security.util.services import set_file_limit
from security.version import VERSION
...
class SecurityApp:
    def __init__(
        self, config: SecurityConfig, manager: SyncManager, stop_event: MpEvent
    ) -> None:
    ...
    def init_database(self) -> None:
        ...
        if len(router.diff) > 0:
            ...
            shutil.copyfile(
                ...
                self.config.database.path.replace("security.db", "backup.db"),
            )
    ...
    def start_watchdog(self) -> None:
        self.security_watchdog = SecurityWatchdog(self.detectors, self.stop_event)
    ...
    def start(self) -> None:
        logger.info(f"Starting Security ({VERSION})")
    ...
    def stop(self) -> None:
        ...
        Path("/dev/shm/.security-is-stopping").touch()
