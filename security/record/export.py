...
from security.config import FfmpegConfig, SecurityConfig
...
from security.ffmpeg_presets import (
    EncodeTypeEnum,
...
class RecordingExporter(threading.Thread):
    ...
    def get_record_export_command(self, video_path: str) -> list[str]:
        ...
        # add metadata
        title = f"Security Recording for {self.camera}, {self.get_datetime_from_timestamp(self.start_time)} - {self.get_datetime_from_timestamp(self.end_time)}"
    ...
    def get_preview_export_command(self, video_path: str) -> list[str]:
        ...
        # add metadata
        title = f"Security Preview for {self.camera}, {self.get_datetime_from_timestamp(self.start_time)} - {self.get_datetime_from_timestamp(self.end_time)}"
        ffmpeg_cmd.extend(["-metadata", f"title={title}"])
