...
from security.api.auth import get_allowed_cameras_for_filter, get_current_user
from security.comms.event_metadata_updater import EventMetadataPublisher
from security.models import Event, Recordings, ReviewSegment, Timeline
from security.stats.emitter import StatsEmitter
from security.test.http_api.base_http_test import BaseTestHttp
from security.test.test_storage import _insert_mock_event
...
class TestHttpApp(BaseTestHttp):
    ...
    def test_get_metrics(self):
        ...
        assert "# TYPE security_detection_total_fps gauge" in event.text
        assert "security_detection_total_fps 13.7" in event.text
        assert (
            "# HELP security_camera_events_total Count of camera events since exporter started"
            in event.text
        )
        assert "# TYPE security_camera_events_total counter" in event.text
        assert (
            'security_camera_events_total{camera="front_door",label="Mock"} 3.0'
            in event.text
        )
        assert (
            'security_camera_events_total{camera="front_door",label="inside"} 2.0'
            in event.text
        )
        assert (
            'security_camera_events_total{camera="front_door",label="outside"} 2.0'
            in event.text
        )
        assert (
            'security_camera_events_total{camera="porch",label="Mock"} 2.0' in event.text
        )
        assert 'security_camera_events_total{camera="porch",label="inside"} 2.0'
