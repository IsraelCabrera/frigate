...
from security.config.camera.ffmpeg import FFMPEG_INPUT_ARGS_DEFAULT
...
class TestFfmpegPresets(unittest.TestCase):
    ...
    def test_default_ffmpeg(self):
        SecurityConfig(**self.default_ffmpeg)

    def test_ffmpeg_hwaccel_preset(self):
        self.default_ffmpeg["cameras"]["back"]["ffmpeg"]["hwaccel_args"] = (
            "preset-rpi-64-h264"
        )
        security_config = SecurityConfig(**self.default_ffmpeg)
        assert "preset-rpi-64-h264" not in (
            " ".join(security_config.cameras["back"].ffmpeg_cmds[0]["cmd"])
        )
    ...
    def test_ffmpeg_hwaccel_scale_preset(self):
        ...
        assert (
            ...
            in (" ".join(security_config.cameras["back"].ffmpeg_cmds[0]["cmd"]))
        )
    ...
    def test_default_ffmpeg_input_arg_preset(self):
        ...
        security_preset_config = SecurityConfig(**self.default_ffmpeg)
        assert (
            # Ignore global and user_agent args in comparison
            security_preset_config.cameras["back"].ffmpeg_cmds[0]["cmd"]
            == security_config.cameras["back"].ffmpeg_cmds[0]["cmd"]
        )
