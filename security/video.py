...
from security.config import CameraConfig, DetectConfig, ModelConfig
...
from security.log import LogPipe
from security.motion import MotionDetector
from security.motion.improved_motion import ImprovedMotionDetector
from security.object_detection.base import RemoteObjectDetector
from security.ptz.autotrack import ptz_moving_at_frame_time
from security.track import ObjectTracker
from security.track.norfair_tracker import NorfairTracker
from security.track.tracked_object import TrackedObjectAttribute
from security.util.builtin import EventsPerSecond
from security.util.image import (
    FrameManager,
    SharedMemoryFrameManager,
    draw_box_with_label,
)
from security.util.object import (
    create_tensor_input,
...
from security.util.time import get_tomorrow_at_time
...
class CameraCapture(SecurityProcess):
    def __init__(
        self,
        config: CameraConfig,
        shm_frame_count: int,
        ...
        super().__init__(
            stop_event,
            PROCESS_PRIORITY_HIGH,
            name=f"security.capture:{config.name}",
            daemon=True,
        )
        self.config = config
        self.shm_frame_count = shm_frame_count
...
class CameraTracker(SecurityProcess):
    def __init__(
        self,
        config: CameraConfig,
        ...
        super().__init__(
            stop_event,
            PROCESS_PRIORITY_HIGH,
            name=f"security.process:{config.name}",
            daemon=True,
        )
        self.config = config
