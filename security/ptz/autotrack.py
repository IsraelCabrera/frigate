...
from security.config import CameraConfig, SecurityConfig, ZoomingModeEnum
...
from security.util.builtin import update_yaml_file_bulk
from security.util.config import find_config_file
from security.util.image import SharedMemoryFrameManager, intersection_over_union
    ...
    def _calculate_move_coefficients(self, camera, calibration=False):
        ...
        if calibration or (
            ...
            if not coefficients_valid:
                logger.warning(
                    f"{camera}: Autotracking calibration failed. See the Security documentation."
                )
                return False
