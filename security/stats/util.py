...
from security.const import CACHE_DIR, CLIPS_DIR, RECORD_DIR
...
from security.types import StatsTrackingTypes
...
def get_latest_version(config: SecurityConfig) -> str:
    if not config.telemetry.version_check:
        return "disabled"

    try:
        request = requests.get(
            "https://api.github.com/repos/blakeblackshear/security/releases/latest",
            timeout=10,
        )
...
def stats_init(
    ...
    stats_tracking: StatsTrackingTypes = {
        ...
        "latest_security_version": get_latest_version(config),
...
def get_processing_stats(
    config: SecurityConfig, stats: dict[str, str], hwaccel_errors: list[str]
) -> None:
...
async def set_bandwidth_stats(config: SecurityConfig, all_stats: dict[str, Any]) -> None:
...
async def set_gpu_stats(
    config: SecurityConfig, all_stats: dict[str, Any], hwaccel_errors: list[str]
) -> None:
...
async def set_npu_usages(config: SecurityConfig, all_stats: dict[str, Any]) -> None:
    stats: dict[str, dict] = {}
...
def stats_snapshot(
    config: SecurityConfig, stats_tracking: StatsTrackingTypes, hwaccel_errors: list[str]
) -> dict[str, Any]:
    ...
    stats["service"] = {
        ...
        "version": VERSION,
        "latest_version": stats_tracking["latest_security_version"],
        "storage": {},
