...
class CustomCollector(object):
    ...
    def add_metric_process(
        ...
        try:
            ...
            try:
                # new security:0.13.0-beta3 stat 'cmdline'
    ...
    def collect(self):
        ...
        audio_dBFS = GaugeMetricFamily(
            "Security_audio_dBFS", "Audio dBFS for camera", labels=["camera_name"]
        )
        audio_rms = GaugeMetricFamily(
            "security_audio_rms", "Audio RMS for camera", labels=["camera_name"]
        )
        ...
        detection_fps = GaugeMetricFamily(
            "security_detection_fps",
            "Number of times detection is run per second.",
            labels=["camera_name"],
        )
        process_fps = GaugeMetricFamily(
            "security_process_fps",
            "Frames per second being processed by security.",
            labels=["camera_name"],
        )
        skipped_fps = GaugeMetricFamily(
            "security_skipped_fps",
            "Frames per second skip for processing by security.",
            labels=["camera_name"],
        )

        # read camera stats assuming version < security:0.13.0-beta3
        cameras = stats
        try:
            # try to read camera stats in case >= security:0.13.0-beta3
        ...
        bandwidth_usages = GaugeMetricFamily(
            "Security_bandwidth_usages_kBps",
        ...
        gpu_usages = GaugeMetricFamily(
            "security_gpu_usage_percent", "GPU utilisation %", labels=["gpu_name"]
        )
        gpu_mem_usages = GaugeMetricFamily(
            "security_gpu_mem_usage_percent", "GPU memory usage %", labels=["gpu_name"]
        )
        ...
        uptime_seconds = GaugeMetricFamily(
            "security_service_uptime_seconds", "Uptime seconds"
        )
        last_updated_timestamp = GaugeMetricFamily(
            "security_service_last_updated_timestamp",
        ...
        try:
            ...
            yield InfoMetricFamily(
                "security_service", "Security version info", value=info
            )
        ...
        temperatures = GaugeMetricFamily(
            "security_device_temperature", "Device Temperature", labels=["device"]
        )
        try:
        ...
        storage_free = GaugeMetricFamily(
            "security_storage_free_bytes", "Storage free bytes", labels=["storage"]
        )
        storage_mount_type = InfoMetricFamily(
            "security_storage_mount_type",
            "Storage mount type",
            labels=["mount_type", "storage"],
        )
        storage_total = GaugeMetricFamily(
            "security_storage_total_bytes", "Storage total bytes", labels=["storage"]
        )
        storage_used = GaugeMetricFamily(
            "security_storage_used_bytes", "Storage used bytes", labels=["storage"]
        )
