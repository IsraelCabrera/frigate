"""Generative AI module for Security."""
...
from security.config import CameraConfig, SecurityConfig, GenAIConfig, GenAIProviderEnum
from security.const import CLIPS_DIR
from security.data_processing.post.types import ReviewMetadata
...
class GenAIClient:
    """Generative AI client for Security."""
...
def get_genai_client(config: SecurityConfig) -> Optional[GenAIClient]:
...
def load_providers():
    ...
    for filename in os.listdir(package_dir):
        if filename.endswith(".py") and filename != "__init__.py":
            module_name = f"security.genai.{filename[:-3]}"
