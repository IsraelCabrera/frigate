...
These are the MQTT messages generated by Security. The default topic_prefix is `security`, but can be changed in the config file.
...
"online": published when Security is running (on startup)
"offline": published after Security has stopped

### `security/restart`

Causes Security to exit. Docker should be configured to automatically restart the container on exit.

### `security/events`

Message published for each changed tracked object. The first message is published when the tracked object is no longer marked as a false_positive. When Security finds a better snapshot of the tracked object or when a zone change occurs, it will publish a message with the same id. When the tracked object ends, a final message is published with `end_time` set.
...
### `security/tracked_object_update`
...
```json
{
  ...
  "before": {
    ...
    "end_time": null,
    "severity": "detection",
    "thumb_path": "/media/security/clips/review/thumb-front_cam-1718987129.308396-fqk5ka.webp",
    "data": {
      "detections": [
        // list of event IDs
...
### `security/camera_activity`

Returns data about each camera, its current features, and if it is detecting motion, objects, etc. Can be triggered by publising to `security/onConnect`

### `security/notifications/set`
...
## Security Camera Topics

### `security/<camera_name>/<role>/status`
...
### `security/<zone_name>/<object_name>`
...
### `security/<zone_name>/<object_name>/active`
...
### `security/<camera_name>/<object_name>/snapshot`
...
### `security/<camera_name>/audio/<audio_type>`
...
### `security/<camera_name>/birdseye_mode/state`

Topic with current state of the Birdseye mode for a camera. Published values are `CONTINUOUS`, `MOTION`, `OBJECTS`.

### `security/<camera_name>/notifications/set`

Topic to turn notifications on and off. Expected values are `ON` and `OFF`.

### `security/<camera_name>/notifications/state`

Topic with current state of notifications. Published values are `ON` and `OFF`.

### `security/<camera_name>/notifications/suspend`

Topic to suspend notifications for a certain number of minutes. Expected value is an integer.

### `security/<camera_name>/notifications/suspended`
