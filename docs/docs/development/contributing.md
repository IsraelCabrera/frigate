...
This repository holds the main Security application and all of its dependencies.

Fork [blakeblackshear/security](https://github.com/blakeblackshear/security.git) to your own GitHub profile, then clone the forked repo to your local machine.
...
### Security Home Assistant Add-on

This repository holds the Home Assistant Add-on, for use with Home Assistant OS and compatible installations. It is the piece that allows you to run Security from your Home Assistant Supervisor tab.

Fork [blakeblackshear/security-hass-addons](https://github.com/blakeblackshear/security-hass-addons) to your own Github profile, then clone the forked repo to your local machine.

### Security Home Assistant Integration

This repository holds the custom integration that allows your Home Assistant installation to automatically create entities for your Security instance, whether you are running Security as a standalone Docker container or as a [Home Assistant Add-on](#security-home-assistant-add-on).

Fork [blakeblackshear/security-hass-integration](https://github.com/blakeblackshear/security-hass-integration) to your own GitHub profile, then clone the forked repo to your local machine.
...
Upon opening, you should be prompted to open the project in a remote container. This will build a container on top of the base Security container with all the development dependencies installed. This ensures everyone uses a consistent development environment without the need to install any dependencies on your host machine.
...
cameras:
  test:
    ffmpeg:
      inputs:
        - path: /media/security/car-stopping.mp4
...
#### 4. Run Security from the command line

VS Code will start the Docker Compose file for you and open a terminal window connected to `security-dev`.
...
- Run `python3 -m security` to start the backend.
...
- All [core](#core) prerequisites _or_ another running Security instance locally available
- Node.js 20

### Making changes

#### 1. Set up a Security instance

The Web UI requires an instance of Security to interact with for all of its data. You can either run an instance locally (recommended) or attach to a separate instance accessible on your network.

To run the local instance, follow the [core](#core) development instructions.

If you won't be making any changes to the Security HTTP API, you can attach the web development server to any Security instance on your network. Skip this step and go to [3a](#3a-run-the-development-server-against-a-non-local-instance).
...
- Do not make large sweeping changes. [Open a discussion on GitHub](https://github.com/blakeblackshear/security/discussions/new) for any large or architectural ideas.
...
The docs are built using [Docusaurus v3](https://docusaurus.io). Please refer to the Docusaurus docs for more information on how to modify Security's documentation.
...
Security uses [Weblate](https://weblate.org) to manage translations of the Web UI. To contribute translation, sign up for an account at Weblate and navigate to the Security NVR project:

https://hosted.weblate.org/projects/security-nvr/
