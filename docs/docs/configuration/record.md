...
Recordings can be enabled and are stored at `/media/security/recordings`. The folder structure for the recordings is `YYYY-MM-DD/HH/<camera_name>/MM.SS.mp4` in **UTC time**. These recordings are written directly from your camera stream without re-encoding. Each camera supports a configurable retention policy in the config. Security chooses the largest matching retention value between the recording retention and the tracked object retention when determining if a recording should be removed.
...
## Will Security delete old recordings if my storage runs out?

As of Security 0.12 if there is less than an hour left of storage, the oldest 2 hours of recordings will be deleted.

## Configuring Recording Retention

Security supports both continuous and tracked object based recordings with separate retention modes and retention periods.
...
Using Security UI, Home Assistant, or MQTT, cameras can be automated to only record in certain situations or at certain times.

## How do I export recordings?

Footage can be exported from Security by right-clicking (desktop) or long pressing (mobile) on a review item in the Review pane or by clicking the Export button in the History view. Exported footage is then organized and searchable through the Export view, accessible from the main navigation bar.
...
In some cases the recordings files may be deleted but Security will not know this has happened. Recordings sync can be enabled which will tell Security to check the file system and delete any db entries for files which don't exist.
...
This feature is meant to fix variations in files, not completely delete entries in the database. If you delete all of your media, don't use `sync_recordings`, just stop Security, delete the `security.db` database, and restart.
