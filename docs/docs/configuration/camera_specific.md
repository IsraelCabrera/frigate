      ...
      inputs:
        - path: rtsp://USERNAME:PASSWORD@CAMERA-IP/H264/ch1/main/av_stream # <----- Update for your camera
          roles:
            ...
            - record
    detect:
      width: # <- optional, by default Security tries to automatically detect resolution
      height: # <- optional, by default Security tries to automatically detect resolution
```
...
[Some users have reported](https://www.reddit.com/r/security_nvr/comments/1hg4ze7/hikvision_security_settings) that newer Hikvision cameras require adjustments to the security settings:
...
Security works much better with newer reolink cameras that are setup with the below options:
...
According to [this discussion](https://github.com/blakeblackshear/security/issues/3235#issuecomment-1135876973), the http video streams seem to be the most reliable for Reolink.
...
Ensure HTTP is enabled in the camera's advanced network settings. To use two way talk with Security, see the [Live view documentation](/configuration/live#two-way-talk).
...
To use a USB camera (webcam) with Security, the recommendation is to use go2rtc's [FFmpeg Device](https://github.com/AlexxIT/go2rtc?tab=readme-ov-file#source-ffmpeg-device) support:

- Preparation outside of Security:
  ...
  - If using Security in a container (e.g. Docker on TrueNAS), ensure you have USB Passthrough support enabled, along with a specific Host Device (`/dev/video0`) + Container Device (`/dev/video0`) listed.

- In your Security Configuration File, add the go2rtc stream and roles as appropriate:
