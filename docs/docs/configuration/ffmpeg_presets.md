...
Some presets of FFmpeg args are provided by default to make the configuration easier. All presets can be seen in [this file](https://github.com/blakeblackshear/security/blob/master/security/ffmpeg_presets.py).

### Hwaccel Presets

It is highly recommended to use hwaccel presets in the config. These presets not only replace the longer args, but they also give Security hints of what hardware is available and allows Security to make other optimizations using the GPU such as when encoding the birdseye restream or when scaling a stream that has a size different than the native stream size.
...
See [the camera specific docs](/configuration/camera_specific.md) for more info on non-standard cameras and recommendations for using them in Security.
...
| preset-rtsp-restream             | RTSP Stream from restream | Use for rtsp restream as source for security                                                      |
| preset-rtsp-restream-low-latency | RTSP Stream from restream | Use for rtsp restream as source for security to lower latency, may cause issues with some cameras |
